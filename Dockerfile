...
ENV WILDFLY_BIN /opt/jboss/wildfly/bin
ENV WILDFLY_CONFIGURATION /opt/jboss/wildfly/standalone/configuration
#ENV JAVA_OPTS
...
ADD configure-elytron.cli $WILDFLY_BIN
ADD jwt.keystore $WILDFLY_CONFIGURATION
...
RUN echo "=> Starting WildFly server" && \
    ...
    echo "=> Adding JWT configuration" && \
      bash -c '$WILDFLY_BIN/jboss-cli.sh --connect --file=$WILDFLY_BIN/configure-elytron.cli' && \
    ...
    echo "=> Creating a new datasource" && \
      $JBOSS_CLI --connect --command="data-source add \
        --name=${DB_NAME}DS \
        --jndi-name=java:jboss/jdbc/${JNDI_NAME}\
        --user-name=${DB_USER} \
        --password=${DB_PASS} \
        --driver-name=mysql \
